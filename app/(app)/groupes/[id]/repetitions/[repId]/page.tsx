...
function SortableSongRow({
  ...
  onPdfClick: (url: string, title: string, kind?: 'pdf' | 'image', songId?: number, songTitle?: string) => void
}) {
              ...
              {song.resources.map((res) => {
                ...
                if (res.type === 'PDF' || res.type === 'IMAGE') {
                  return (
                    <div key={res.id}>
                      <button
                        onClick={() => onPdfClick(`/api/ressources/${res.id}`, res.name, res.type === 'IMAGE' ? 'image' : 'pdf', song.id, song.title)}
...
export default function RepetitionDetailPage({ params }: { params: { id: string; repId: string } }) {
  ...
  const isFounder = isChef && (session?.user?.siteRole === 'ADMIN' || myUserId === groupInfo?.createdBy)
                  ...
                  <div className="space-y-3">
                    {songs.map((entry, index) => (
                      <SortableSongRow
                        ...
                        onPdfClick={(url, title, kind, songId, songTitle) => setPdfModal({ url, title, kind, songId, songTitle })}
                      />
                    ))}
                  </div>
                </SortableContext>
