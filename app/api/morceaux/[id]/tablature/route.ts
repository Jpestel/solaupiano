...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const tab = await prisma.songTab.findUnique({ where: { songId } })
  return NextResponse.json(tab ?? { content: null })
}
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const tab = await prisma.songTab.upsert({
    where: { songId },
    update: { content },
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await prisma.songTab.deleteMany({ where: { songId } })
