...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const planRec = await prisma.plan.findUnique({
    ...
    select: { hasMetronome: true, hasParoles: true, hasSequences: true, hasEvaluations: true, hasGrilles: true, hasSetlists: true, hasConcerts: true, hasFicheTechnique: true, hasMaPage: true, hasStats: true },
  })
  const planFeatures = {
