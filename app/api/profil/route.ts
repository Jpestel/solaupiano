...
export async function PATCH(req: NextRequest) {
  ...
  const userId = Number(session.user.id)
  const { name, instrumentIds, userPlan, weeklyDigestOptOut, rehearsalReminderOptOut, evaluationReminderOptOut, gusoNumber, stageFigure, stageColor, stageName } = await req.json()
  ...
  await prisma.$transaction([
    prisma.user.update({
      where: { id: userId },
      data: { name: name.trim(), ...planData, ...digestData, ...reminderData, ...evalReminderData, ...gusoData, ...figureData, ...stageColorData, ...stageNameData },
    }),
